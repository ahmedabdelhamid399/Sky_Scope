package com.iti.skyscope.home.view
...
import com.iti.skyscope.broadcastreceiver.ConnectivityReceiver
import com.iti.skyscope.datasource.MyLocationProvider
import com.iti.skyscope.datasource.WeatherRepository
import com.iti.skyscope.datasource.model.Daily
import com.iti.skyscope.datasource.model.Hourly
import com.iti.skyscope.datasource.model.OpenWeatherApi
import com.iti.skyscope.home.viewmodel.HomeViewModel
import com.iti.skyscope.home.viewmodel.HomeViewModelFactory
import com.iti.skyscope.util.convertCalenderToDayString
import com.iti.skyscope.util.convertLongToDayDate
import com.iti.skyscope.util.convertNumbersToArabic
import com.iti.skyscope.util.getCityText
import com.iti.skyscope.util.getCurrentLocale
import com.iti.skyscope.util.getIcon
import com.iti.skyscope.util.getSharedPreferences
import com.iti.skyscope.util.isSharedPreferencesLatAndLongNull
import com.iti.skyscope.util.isSharedPreferencesLocationAndTimeZoneNull
import com.iti.skyscope.util.updateSharedPreferences
import com.iti.weatherwatch.R
import com.iti.weatherwatch.databinding.FragmentHomeBinding
import java.util.Calendar

/*
This class is the implementation of the HomeFragment in the WeatherWatch app.
It is a subclass of the Fragment class and is responsible for displaying the main view of the app,
including the weather forecast for the user's current location, as well as handling various events,
such as network connectivity changes and user interactions.

The class also includes the onCreateView(), onResume(), onViewCreated() methods,
which are lifecycle methods of a fragment in Android.
In the onCreateView() method, the fragment's layout is inflated, and the view is returned.

In the onResume() method, the ConnectivityReceiverListener is set to the class instance
to listen to changes in network connectivity.

In the onViewCreated() method, various views and adapters are initialized,
and the current location is obtained to display weather information.
This method also observes changes in location and weather data and updates the UI accordingly.

This class has a number of properties and methods for initializing and interacting with views and data.
Some of the notable properties include:

    tempPerDayAdapter and tempPerTimeAdapter are instances of TempPerDayAdapter and TempPerTimeAdapter,
    respectively, which are adapters used to display data in RecyclerViews.
    windSpeedUnit and temperatureUnit are strings representing the current wind speed
    and temperature units of the weather data being displayed.
    latitude, longitude, language, and units are properties that hold data relevant to the current location
    and weather data being displayed.
...
*/
/*
 Summary, this class represents the functionality of the home screen of a weather app and
 includes methods for displaying weather data and handling changes in network connectivity.
 */
class HomeFragment : Fragment(), ConnectivityReceiver.ConnectivityReceiverListener
{
    ...
    private val homeViewModel: HomeViewModel by viewModels {
        HomeViewModelFactory(
            ...
            //getRepository() method is a singleton that provides access to weather data
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (!flagNoConnection)
        {
            if (isSharedPreferencesLocationAndTimeZoneNull(requireContext()))
            {
                if (!isSharedPreferencesLatAndLongNull(requireContext()))
                {
                    ...
                    try
                    {
                        homeViewModel.getDataFromRemoteToLocal(
                            "$latitude",
                            "$longitude",
                            language,
                            units
        ...
        homeViewModel.observeLocation().observe(viewLifecycleOwner) {
        ...
        homeViewModel.openWeatherAPI.observe(viewLifecycleOwner) {
            if (it != null) {
    ...
    //display temperature data per hour in a horizontal scrollable list
    ...
    //display temperature data per day
    ...
    private fun setDataFromSharedPreferences() {
    ...
    override fun onNetworkConnectionChanged(isConnected: Boolean) {
        if (fragmentHomeBinding != null) {
            if (isConnected) {
                if (flagNoConnection) {
                    val snackBar = Snackbar.make(binding.root, "Connected", Snackbar.LENGTH_SHORT)
